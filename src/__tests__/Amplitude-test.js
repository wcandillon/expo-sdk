...
describeCrossPlatform('all Amplitude methods available', () => {
  it('initializes', () => {
    expect(Amplitude.initialize()).toBeUndefined();
  ...
  it('can setUserId', () => {
    ...
    expect(NativeModules.ExponentAmplitude.setUserId).toHaveBeenCalledWith(
      'user-id'
    );
  });
  ...
  it('can setUserProperties', () => {
    ...
    expect(
      NativeModules.ExponentAmplitude.setUserProperties
    ).toHaveBeenCalledWith({ some: 'property' });
  });

  it('can clearUserProperties', () => {
    expect(Amplitude.clearUserProperties()).toBeUndefined();
    expect(
      NativeModules.ExponentAmplitude.clearUserProperties
    ).toHaveBeenCalledTimes(1);
  });

  it('can logEvent', () => {
    expect(Amplitude.logEvent('event-name')).toBeUndefined();
    expect(NativeModules.ExponentAmplitude.logEvent).toHaveBeenCalledWith(
      'event-name'
    );
  });
  ...
  it('can setGroup', () => {
    ...
    expect(
      NativeModules.ExponentAmplitude.setGroup
    ).toHaveBeenCalledWith('group', ['group', 'names', 'array']);
  });
});
